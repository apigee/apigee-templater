...
process.on("uncaughtException", function (e) {
  if (
    ...
    console.error(
      `${chalk.redBright("! Error: No project with Apigee enabled was found to deploy to, either set a default project using gcloud or pass the -p parameter with a valid project Id.")}`,
    );
  } else
    console.error(
...
export class cli {
  ...
  printHelp() {
    ...
    console.log(
      `> apigee-templater -n TestProxy -b /httpbin -t https://httpbin.org -d -e test1' ${chalk.italic(chalk.magentaBright("# Create a proxy called TestProxy and deploy to the Apigee X environment 'test1'."))}`,
    );
    console.log(
      `> apigee-templater -f ./PetStore.yaml -d -e test1' ${chalk.italic(chalk.magentaBright("# Create a proxy based on the PetStore.yaml file and deploy to environment 'test1'"))}`,
    );
  ...
  async process(args: string[]) {
    if (!stdin.setRawMode) {
      ...
      for (let argIndex = 0; argIndex < args.length; argIndex++) {
        ...
        else if (args[argIndex] === "-ad") addDescriptions = true;
        else if (args[argIndex] === "-ae") addExamples = true;
        else if (args[argIndex] === "-a") auth = authTypes.apiKey;
      }

      let specResult = await this.apigeeTemplater.generateSpec(
        payloadInput,
        SpecType.Data,
        servers,
        auth,
    ...
    if (options.verbose)
      this.logVerbose(JSON.stringify(options, null, 2), "start:");
    ...
    if (options.file) {
      ...
      if (options.file.startsWith("https://")) {
        // Remote file, fetch..
        if (options.verbose)
          this.logVerbose(`Fetching remote file ${options.file}`, "env:");
    ...
    if (!options.input && !options.file) {
      ...
      try {
        ...
        console.error(
          `${chalk.redBright("! Error:")} Error during prompt for inputs.`,
        );
        if (options.verbose)
    ...
    if (options.verbose)
      this.logVerbose(`Project set to ${process.env.PROJECT}`, "project:");
    ...
    this.apigeeTemplater
      .convertStringToProxyInput(options.input)
      .then((inputTemplate: ApigeeTemplateInput) => {
        if (options.basePath)
          inputTemplate.endpoints[0].basePath = options.basePath;
        if (options.name) inputTemplate.name = options.name;

        this.apigeeTemplater
          .generateProxy(inputTemplate, _proxyDir)
          .then((generateResult: GenerateResult) => {
            if (options.verbose)
              this.logVerbose(
            ...
            if (
              ...
              console.log(
                `${chalk.green(">")} Proxy ${chalk.bold(chalk.blue(generateResult.template.name))} generated to ${chalk.magentaBright(chalk.bold(generateResult.localPath))} in ${chalk.bold(chalk.green(Math.round(generateResult.duration) + " milliseconds"))}.`,
              );

            if (options.deploy && !options.environment) {
              console.error(
                `${chalk.redBright("! Error:")} No environment found to deploy to, please pass the -e parameter with an Apigee X environment.`,
              );
              ...
              try {
                if (generateResult && generateResult.template) {
                  if (inputTemplate.sharedFlow) {
                    this.apigeeService
                      .updateFlow(
                        generateResult.template.name,
                        _proxyDir + "/" + generateResult.template.name + ".zip",
                      )
                      .then((updateResult: ProxyRevision) => {
                          ...
                          if (generateResult && generateResult.template) {
                            this.apigeeService
                              .deployFlowRevision(
                                ...
                                options.deployServiceAccount,
                              )
                              .then((deployResult: ProxyDeployment) => {
                                ...
                                if (generateResult && generateResult.template) {
                                  console.log(
                                    `${chalk.green(">")} Flow ${chalk.bold(chalk.blue(generateResult.template.name + " version " + updateResult.revision))} deployed to environment ${chalk.bold(chalk.magentaBright(options.environment))} in ${chalk.bold(chalk.green(Math.round(duration) + " milliseconds"))}.`,
                                  );
                                }
                              });
                          }
                      ...
                      .catch((error) => {
                        console.error(
                          `${chalk.redBright("! Error:")} Error deploying flow revision, probably a basepath conflict with an existing proxy.`,
                        );
                        if (options.verbose)
                          this.logVerbose(
                    ...
                    this.apigeeService
                          ...
                          if (generateResult && generateResult.template) {
                            this.apigeeService
                              ...
                              .then((deployResult: ProxyDeployment) => {
                                ...
                                if (generateResult && generateResult.template) {
                                  console.log(
                                    `${chalk.green(">")} Proxy ${chalk.bold(chalk.blue(generateResult.template.name + " version " + updateResult.revision))} deployed to environment ${chalk.bold(chalk.magentaBright(options.environment))} in ${chalk.bold(chalk.green(Math.round(duration) + " milliseconds"))}.`,
                                  );

                                  // Now try to get the env group URL
                                  this.apigeeService
                                    .getEnvironmentGroups()
                                    .then((result: EnvironmentGroup[]) => {
                                      result.forEach(
                                        (group: EnvironmentGroup) => {
                                          this.apigeeService
                                            ...
                                            .then((result) => {
                                              result.forEach(
                                                (
                                                  element: EnvironmentGroupAttachment,
                                                ) => {
                                                  if (
                                                    element.environment ===
                                                    options.environment
                                                    ...
                                                    group.hostnames.forEach(
                                                      (hostname: string) => {
                                                        if (
                                                          generateResult.template
                                                        )
                                                          console.log(
                                                            `${chalk.green(">")} Wait 30-60 seconds, then test here: ${chalk.bold(chalk.blue(`https://${hostname}${generateResult.template.endpoints[0].basePath}`))}`,
                                                          );
                                                      },
                                                    );
                                                  }
                              ...
                              .catch((error) => {
                                console.error(
                                  `${chalk.redBright("! Error:")} Error deploying proxy revision, probably a basepath conflict with an existing proxy.`,
                                );
                                if (options.verbose)
                                  this.logVerbose(
                                    JSON.stringify(error),
                      ...
                      .catch((error) => {
                        if (
                          ...
                          console.error(
                            `${chalk.redBright("! Error:")} Error in proxy bundle definition, try importing manually for more detailed error information.`,
                          );
                        } else {
                          console.error(
                            `${chalk.redBright("! Error:")} Error updating proxy.`,
                          );
                        }
                      });
                  }
                ...
                console.error(
                  `${chalk.redBright("! Error:")} Error generating proxy.`,
                );
              }
            }
          });
  ...
  logVerbose(input: string, label?: string) {
    ...
    if (label) logString = "> " + label + " " + input + "\n";
...
const helpCommands = [
  {
    name: "--file, -f",
    description:
      "Path to a JSON or OpenAPIv3 YAML file with a proxy definition.",
  },
  {
    name: "--input, -i",
    description:
      "Same as --file, but with the input directly as a string in this parameter.",
  },
  {
    name: "--deploy, -d",
    description:
      "Boolean true or false if the generated proxy should also be deployed to an Apigee X environment.",
  },
  {
    name: "--deployServiceAccount, -s",
    description:
      "The Google Cloud service account email address to deploy with (used to authenticate or authorize target calls).",
  },
  {
    name: "--environment, -e",
  ...
  },
  {
    name: "--project, -p",
    description:
      "The Google Cloud project / Apigee Org to use for the deployment.",
  },
  {
    name: "--filter, -l",
    description:
      "Path to an optional javascript file that will be evaluated before any processing is done, can be used to add conversion plugins or inject other logic into the conversion.",
  },
  {
    name: "--name, -n",
    description:
      "If no --file or --input parameters are specified, this can set the proxy name directly for a simple proxy.",
  },
  {
    name: "--basePath, -b",
    description:
      "If no --file or --input parameters are specified, this can set the basePath directly.",
  },
  {
    name: "--apiKey, -a",
    description:
      "If no --file or --input parameters are specified, this can apply an apiKey policy when set to true.",
  },
  {
    name: "--targetUrl, t",
    description:
      "If no --file or --input parameters are specified, this can set the target URL directly.",
  },
  {
    name: "--targetBigQueryTable, q",
    description:
      "If no --file or --input parameters are specified, this can set a target BigQuery table directly.",
  },
  {
    name: "--verbose, -v",
    description:
      "If extra logging information should be printed during the conversion and deployment.",
  },
  {
    name: "--datSpec, -ds",
    description:
      "Generates an OpenAPI spec for a data payload that is piped to the command (only GET).",
  },
  {
    name: "--keyPath, -k",
    description:
      "If no GOOGLE_APPLICATION_CREDENTIALS are set to authorize the proxy deployment, this can point to a GCP service account JSON key file to use for authorization.",
  },
];
