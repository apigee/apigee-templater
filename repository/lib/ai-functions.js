  ...
  if (parsedConfig && parsedConfig.models) {
    ...
    if (!result.targetRoute) {
      for (var modelKey in parsedConfig.models) {
        if (currentModel.includes(modelKey)) {
...
function getPrompts(contentData) {
  ...
  if (contentData["contents"] && Array.isArray(contentData["contents"])) {
    ...
    for (var i = contentData["contents"].length - 1; i >= 0; i--) {
      ...
      if (
        ...
        for (var p = content["parts"].length - 1; p >= 0; p--) {
          ...
          if (!promptInfo.userPrompt && part && part["text"]) {
            promptInfo.userPrompt = part["text"];
          }

          if (part && part["text"]) {
            promptInfo.allUserPrompts += " " + part["text"];
          }
        }
      }
    }
    ...
    if (contentData["system"] !== undefined || contentData["anthropic_version"] !== undefined || contentData["top_k"] !== undefined) {
      promptInfo.protocol = "anthropic";
    } else {
      promptInfo.protocol = "openai";
    }
      ...
      if (message && message["role"] && message["role"].toLowerCase() == "user") {
        ...
        if (typeof message["content"] === "string") {
          ...
          for (var c = 0; c < message["content"].length; c++) {
            var item = message["content"][c];
            if (item && item["type"] === "text" && item["text"]) {
              msgText += (msgText ? " " : "") + item["text"];
        ...
        if (!promptInfo.userPrompt && msgText) {
          promptInfo.userPrompt = msgText;
        }

        if (msgText) {
          promptInfo.allUserPrompts += " " + msgText;
        }
      }
    }
    ...
    if (typeof contentData["prompt"] === "string") {
      promptInfo.userPrompt = contentData["prompt"];
      promptInfo.allUserPrompts = contentData["prompt"];
    }
  }
...
function convertAnthropicStreamToOpenAi(contentString, modelName) {
  var usageData = {
    model: modelName || "",
    requestTokenCount: 0,
    responseTokenCount: 0,
  ...
  if (!contentString) {
    return { contentString: contentString || "", usageData: usageData };
  }

  var raw = contentString.trim();

  if (raw.indexOf("message_stop") !== -1 || raw.indexOf("[DONE]") !== -1) {
    return { contentString: "data: [DONE]\n\n", usageData: usageData };
  }

  var jsonStr = raw;
  if (jsonStr.indexOf("data: ") !== -1) {
    var parts = jsonStr.split("data: ");
  ...
  try {
    ...
    var model = modelName || "claude";

    if (eventData.type === "message_start" && eventData.message) {
      var msgId = eventData.message.id ? ("chatcmpl-" + eventData.message.id.replace(/^msg_/, "")) : ("chatcmpl-" + Math.random().toString(36).substring(2, 11));
      var startChunk = {
        id: msgId,
      ...
      return {
        contentString: "data: " + JSON.stringify(startChunk) + "\n\n",
        usageData: usageData
      };
    }

    if (eventData.type === "content_block_delta" && eventData.delta && eventData.delta.text !== undefined) {
      var deltaChunk = {
        id: "chatcmpl-" + Math.random().toString(36).substring(2, 11),
        object: "chat.completion.chunk",
        created: created,
      ...
      return {
        contentString: "data: " + JSON.stringify(deltaChunk) + "\n\n",
        usageData: usageData
      };
    }

    if (eventData.type === "message_delta" && eventData.delta) {
      ...
      return {
        contentString: "data: " + JSON.stringify(stopChunk) + "\n\n",
        usageData: usageData
      };
    }
  } catch (e) {
    // Return unchanged if parsing fails
  }
